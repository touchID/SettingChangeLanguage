...
- (void)setRadius:(CGFloat)radius {
    if (radius <= 0) {
        radius = self.frame.size.width * 0.5f;
    }
...
- (void)addShadow {
    ...
    self.layer.shadowPath = [UIBezierPath bezierPathWithRect:CGRectMake(0, self.frame.size.height - 2, self.frame.size.width == 320 ? [UIScreen mainScreen].bounds.size.width : self.frame.size.width, 2)].CGPath;
}
